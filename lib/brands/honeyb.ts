...
export const honeyb: BrandConfig = {
  ...
  // Only the bundles that exist — omitting one hides its buttons rather than
  // shipping a 404. Zips are generated at build time, not committed.
  downloads: {
    logos: `${B}/downloads/honeyb-logos.zip`,
    tokens: `${B}/downloads/honeyb-tokens.css`,
    // Built at deploy time from the logos, every asset on the page and the
    // tokens — see scripts/build-zips.mjs. Not committed, so size isn't a git cost.
    kit: `${B}/downloads/honeyb-brand-kit.zip`,
  },
  downloadLabels: { logos: "Logo system", tokens: "Design tokens (CSS)", kit: "Full brand kit" },
  sections: [
    { id: "logo", label: "Logo" },
