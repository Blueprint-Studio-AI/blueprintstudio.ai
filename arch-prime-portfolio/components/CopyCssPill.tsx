...
function stylesheet() {
  const out: string[] = ["/* Arch Prime — Type System */", ":root {"];
  for (const [key, face] of Object.entries(TYPE)) out.push(`  --prime-font-${key}: ${face.css};`);
  out.push("}");

  for (const [key, face] of Object.entries(TYPE)) {
    out.push("", `/* ${key === "display" ? "Display" : key === "numbers" ? "Numbers" : "Text"} — ${face.css.split(",")[0].replace(/"/g, "")} */`);
    for (const [label, size, lh, ls] of face.rows) {
      out.push(
        `.prime-${slug(label)} {`,
        `  font-family: var(--prime-font-${key});`,
        `  font-size: ${size}px;`,
        `  line-height: ${lh};`,
        `  letter-spacing: ${ls};`,
        `}`,
