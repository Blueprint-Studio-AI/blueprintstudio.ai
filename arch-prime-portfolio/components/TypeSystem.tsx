...
// Type system (Figma 298:936) — two live, editable specimens (Gascogne display +
...
type GroupKey = keyof typeof TYPE;
...
function TypeGroup({ group, tag, serif, fontClass }: { group: GroupKey; tag: string; serif: boolean; fontClass?: string }) {
  const face_cls = fontClass ?? (serif ? "font-serif" : "font-sans font-normal");
          ...
          <div className="flex w-[596px] shrink-0 flex-col gap-16 max-[1200px]:w-[46%] max-[1024px]:w-full">
            ...
            <div className="flex min-h-[70px] items-start max-[860px]:min-h-[46px]">
              <div
                ...
                className={`caret w-fit max-w-full cursor-text overflow-hidden whitespace-nowrap pr-0.5 leading-[1.1] text-[#181818] caret-[#181818] outline-none max-[860px]:!text-headline-mobile ${face_cls} ${
                  group === "numbers" ? "tabular-nums" : ""
                }`}
            ...
            <div className={`flex flex-col ${serif ? "gap-4" : ""}`}>
              {face.rows.map((row, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`leading-none text-ink ${serif ? "font-serif tracking-[-0.025em]" : fontClass ?? "font-sans"}`}
...
export default function TypeSystem() {
  return (
    <section className="flex flex-col gap-32 px-edge pb-section pt-16">
      <TypeGroup group="display" tag="prime-display" serif />
      <TypeGroup group="text" tag="prime-text" serif={false} />
      <TypeGroup group="numbers" tag="prime-numbers" serif={false} fontClass="font-numbers font-normal" />
    </section>
  );
}
