...
export default function AssetGenerator({ category }: { category: AssetCategory }) {
  ...
  const learnMore = `${ASSET_GEN}?brand=arch&category=${category.id}`;
          ...
          <div className="flex w-full flex-col gap-16 max-[860px]:gap-10">
            ...
            <div className="flex w-full flex-col gap-6">
              <h3 className="text-promo font-medium text-ink max-[860px]:text-title">
                <span className="text-faint">Need something new?</span>{" "}
                Generate more on-brand assets.
              </h3>
              <p className="text-body-lg text-body">
                A model trained on Arch&apos;s palette, type, and textures turns a prompt into imagery that fits the
                system. Built for the whole team: request access to the shared account and start generating.
              </p>
            </div>
          </div>
