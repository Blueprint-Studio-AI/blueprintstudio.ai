...
// Accent colours — the only tokens arch.network actually names beyond the
// primaries (--color-dark-purple, --color-purple, --color-light, plus the two
// oranges/golds it uses inline). No derived ramps: those were guesses.
// Chips carry their step number, lift on hover, and confirm a copy with a
// small centred checkmark.
import { useEffect, useRef, useState } from "react";
import { ACCENTS, type Step } from "@/lib/data";
...
function Chip({
  ...
  const ink = relLum(hex) > 0.5 ? "#181818" : "rgba(255,255,255,0.94)";
  return (
    <button
