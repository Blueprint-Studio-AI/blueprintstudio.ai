  ...
  const content = (
    <div className="p-4 rounded-lg border border-primary/10 hover:border-primary/20 hover:bg-primary/5 transition-colors duration-300">
      <div className="flex items-center justify-between">
        <div>
          <h4 className={`text-base font-medium ${href ? 'group-hover:text-primary' : ''} transition-colors duration-300`}>
            {name}
          </h4>
          <p className={`text-sm text-muted-foreground ${href ? 'group-hover:text-foreground/80' : ''} transition-colors duration-300`}>
            {description}
          </p>
        </div>
        {href && (
